package ca.concordia.qpi.filter;
...
public class MultiscaleVesselFilter {
	...
	private float[] locationImage = null;
						...
						while(dist<=rOut){	
							if(x+s*orthVector[X]>=0 && x+s*orthVector[X]<nx && (y+s*orthVector[Y])>=0 && (y+s*orthVector[Y])<ny && (z+s*orthVector[Z])>=0 && (z+s*orthVector[Z])<nz){
								if(dist>rIn){
									...
									if(!objdiff[x+s*orthVector[X]+nx*(y+s*orthVector[Y])+nx*ny*(z+s*orthVector[Z])])	maskOut[x+s*orthVector[X]+nx*(y+s*orthVector[Y])+nx*ny*(z+s*orthVector[Z])]=true;
								}
								else {
									...
									if(!objdiff[x+s*orthVector[X]+nx*(y+s*orthVector[Y])+nx*ny*(z+s*orthVector[Z])])	maskVote[x+s*orthVector[X]+nx*(y+s*orthVector[Y])+nx*ny*(z+s*orthVector[Z])]=true;
								}
							}
							if(x-s*orthVector[X]>=0 && x+s*orthVector[X]<nx && (y-s*orthVector[Y])>=0 && (y-s*orthVector[Y])<ny && (z-s*orthVector[Z])>=0 && (z-s*orthVector[Z])<nz){
								if(dist>rIn){
...
//		return;
	...
	private final void shapeFromRecursiveRidgeFilter( float[] filter, float[] shape) {
		...
		System.out.println("parameter estimates: max "+max+", median "+median+", beta "+beta+",\n");
		...
		for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++){
			...
			if (filter[xyz]>0) {
                ...
                if (shape[xyz]<pmin) pmin = shape[xyz];
                if (shape[xyz]>pmax) pmax = shape[xyz];
			}
		}
		System.out.println("probabilities: ["+pmin+", "+pmax+"]\n");
		return;
	}
