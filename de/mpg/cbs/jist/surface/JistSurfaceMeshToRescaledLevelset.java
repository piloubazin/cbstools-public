...
public class JistSurfaceMeshToRescaledLevelset  extends ProcessingAlgorithm{
	ParamVolume origVol;
	ParamVolume rescaleVol;
	...
	protected void createInputParameters(ParamCollection inputParams) {
		...
		inputParams.add(origVol=new ParamVolume("Reference Image Volume"));
		origVol.setDescription("Reference volume to use for the original image dimensions and resolutions.");
		inputParams.add(rescaleVol=new ParamVolume("Reference Rescaled Volume"));
		rescaleVol.setDescription("Reference volume to use for the output levelset dimensions and resolutions.");
		...
		inputParams.setName("MeshToRescaledLevelSet");
		inputParams.setLabel("Mesh to Rescaled Level Set");
		...
		info.setDescription("Finds a volumetric level-set representation of an input surface in a rescaled space.");
	...
	protected void execute(CalculationMonitor monitor) {
		...
		int nix = origVolData.getRows();
		int niy = origVolData.getCols();
		int niz = origVolData.getSlices();
		float rix = origVolData.getHeader().getDimResolutions()[0];
		float riy = origVolData.getHeader().getDimResolutions()[1];
		float riz = origVolData.getHeader().getDimResolutions()[2];
		float oix = origVolData.getHeader().getOrigin()[0];
		float oiy = origVolData.getHeader().getOrigin()[1];
		float oiz = origVolData.getHeader().getOrigin()[2];
		
		ImageDataFloat	rescaleVolData = new ImageDataFloat(rescaleVol.getImageData());
		int nrx = rescaleVolData.getRows();
		int nry = rescaleVolData.getCols();
		int nrz = rescaleVolData.getSlices();
		float rrx = rescaleVolData.getHeader().getDimResolutions()[0];
		float rry = rescaleVolData.getHeader().getDimResolutions()[1];
		float rrz = rescaleVolData.getHeader().getDimResolutions()[2];
		float orx = rescaleVolData.getHeader().getOrigin()[0];
		float ory = rescaleVolData.getHeader().getOrigin()[1];
		float orz = rescaleVolData.getHeader().getOrigin()[2];
		...
		if (mipavTransform.getValue().booleanValue()) {
			...
			Point3f pt0 = new Point3f((nix-1)*rix/2.0f, (niy-1)*riy/2.0f, (niz-1)*riz/2.0f);
			surf.translate(pt0);
		}
		EmbeddedSurface rsurf = surf.clone();
		
		// scale the surface data into voxel space
		rsurf.scaleVertices(new float[]{1.0f/rrx, -1.0f/rry, -1.0f/rrz});
		Point3f pt1 = new Point3f(0, (nry-1), (nrz-1));
		rsurf.translate(pt1);
		...
		*/
		...
		hash = new MeshDistanceHash(mesh, 1.0,	new Point3f(0, 0, 0), new Point3f(nrx, nry, nrz));
		...
		df.setHeader(rescaleVolData.getHeader());
		df.setName(surf.getName()+"_rls");
		result.setValue(df);
		
		if (mipavTransform.getValue().booleanValue()) {
			surf.setName(surf.getName()+"_ral");
