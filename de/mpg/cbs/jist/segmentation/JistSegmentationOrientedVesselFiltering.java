...
public class JistSegmentationOrientedVesselFiltering extends ProcessingAlgorithm {
	...
	private ParamFloat surfdistParam;
	//private ParamOption orientParam;
	//private static final String[] orientTypes = {"parallel","orthogonal"};
	private ParamInteger orientfactorParam;
	...
	*/
	...
	//private	static 	final 	byte 	ORTHOGONAL = 101;
	//private	static 	final 	byte 	PARALLEL = 102;
	...
	protected void createInputParameters(ParamCollection inputParams) {
		...
		inputParams.add(surfaceImage = new ParamVolume("Surface or laminar Level Set(s)",null,-1,-1,-1,-1));
		inputParams.add(surfdistParam = new ParamFloat("Max surface distance", 0.0f, 100.0f, 1.0f));
		inputParams.add(orientfactorParam = new ParamInteger("Orientation preference factor ", 1, 10, 3));
		//inputParams.add(orientParam = new ParamOption("Vessel orientation", orientTypes));
		...
		//inputParams.add(thresholdParamProba = new ParamFloat("In Probability threshold", 0.0, 1.0, 0.5));
		//inputParams.add(thresholdReg = new ParamInteger("Clean Estimation threshold", 0, 50, 15));
		//inputParams.add(thresholdParamPv = new ParamFloat("Partial Voluming threshold", 0.0, 1.0, 0.7));
		...
		inputParams.setLabel("Oriented Vessel Filtering");
		inputParams.setName("OrientedVesselFiltering");
	...
	protected void createOutputParameters(ParamCollection outputParams) {
		...
		*/
		...
		outputParams.setName("Oriented vessel filtering");
		outputParams.setLabel("Oriented vessel filtering");
	}
	...
	protected void execute(CalculationMonitor monitor){
		...
		ImageDataFloat surfImg = new ImageDataFloat(inputImage.getImageData());
		nsurf = surfImg.getComponents();
		float[][] surfaces = new float[nsurf][nxyz];
		if (nsurf==1) {
			float[][][] buffer3 = surfImg.toArray3d();
			for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++) {
				int xyz = x+nx*y+nx*ny*z;
				surfaces[0][xyz] = buffer3[x][y][z];
			}
			buffer3 = null;
		} else {
			float[][][][] buffer4 = surfImg.toArray4d();
			for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++) for (int l=0;l<nsurf;l++) {
				int xyz = x+nx*y+nx*ny*z;
				surfaces[l][xyz] = buffer4[x][y][z][l];
			}
			buffer4 = null;
		}
		surfImg = null;
		
		float surfDist = surfdistParam.getValue().floatValue();
		//byte orient = ORTHOGONAL;
		//if (orientParam.getValue().equals("parallel")) orient = PARALLEL;
		int orientfactor = orientfactorParam.getValue().intValue();
		...
		for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++) {
			...
			// mask away from image
			if (surfaces[0][id]<-surfDist) maskCort[id] = false;
			if (surfaces[nsurf-1][id]>surfDist) maskCort[id] = false;
		...
		float[][] scaleProduct = new float[scaleNbr][nxyz];
		for(int i=0;i<scaleNbr;i++){
			...
			for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++) {
				...
				// mask away from image
				if (surfaces[0][id]<-surfDist) mask[id] = false;
				if (surfaces[nsurf-1][id]>surfDist) mask[id] = false;
				// remove border from computations
			...
			//if (orient==ORTHOGONAL) normalDirectionFromRecursiveRidgeFilter(smoothed, mask, surfaces, filter, direction);
			//else if (orient==PARALLEL) parallelDirectionFromRecursiveRidgeFilter(smoothed, mask, surfaces, filter, direction);
			//}
			normalDirectionFromRecursiveRidgeFilter(smoothed, mask, surfaces, filter, direction, product, orientfactor);
			
			response[i]=filter;	
			scaleDirection[i]=direction;
			scaleProduct[i]=product;
		}
		
		//Combine scales with orientation
		float[] finalResponse = response[0];
		float[] scaleMax = new float[nxyz];
		for(int id=0;id<nxyz;id++){
			finalResponse[id] = response[0][id]*scaleProduct[0][id];
			scaleMax[id]=scaleFactors[0];
			for(int i=1;i<scaleNbr;i++){
				if (response[i][id]*scaleProduct[i][id]>finalResponse[id]) {
					finalResponse[id]=response[i][id]*scaleProduct[i][id];
		...
		float[] scaleMax = new float[nxyz];
		byte[] finalDirection= scaleDirection[0];
		for(int id=0;id<nxyz;id++) if (maskCort[id]) {
			// estimate orientation
			int s0=0;
			for (int s=1;s<nsurf;s++) {
				if (Numerics.abs(surfaces[s][id])<Numerics.bas(surfaces[s0][id])) s0=s;
			}
			double vx = 0.5*(surfaces[s0][Ngb.neighborIndex(Ngb.pX, id, nx,ny,nz)]-surfaces[s0][Ngb.neighborIndex(Ngb.mX, id, nx,ny,nz)]);
			double vy = 0.5*(surfaces[s0][Ngb.neighborIndex(Ngb.pY, id, nx,ny,nz)]-surfaces[s0][Ngb.neighborIndex(Ngb.mY, id, nx,ny,nz)]);
			double vz = 0.5*(surfaces[s0][Ngb.neighborIndex(Ngb.pZ, id, nx,ny,nz)]-surfaces[s0][Ngb.neighborIndex(Ngb.mZ, id, nx,ny,nz)]);
			double norm = FastMath.sqrt(vx*vx+vy*vy+vz*vz);
			vx /= norm; vy /= norm; vz /= norm;
			
			// scale by product
			float[] dirvect = Ngb.directionVector(scaleDirection[0][id]);
			double score = FastMath.acos(Numerics.abs(vx*dirvect[0]+vy*dirvect[1]+vz*dirvect[2]));
			if (direction==PARALLEL) score =  Math.PI/2.0-score;
			finalResponse[id] *= score;
			
			scaleMax[id]=scaleFactors[0];
			for (int i=1;i<scaleNbr;i++) {
				dirvect = Ngb.directionVector(scaleDirection[i][id]);
				score = FastMath.acos(Numerics.abs(vx*dirvect[0]+vy*dirvect[1]+vz*dirvect[2]));
				if (direction==PARALLEL) score =  Math.PI/2.0-score;
				
				if (score*response[i][id]>finalResponse[id]) {
					finalResponse[id] = score*response[i][id];
					scaleMax[id]=scaleFactors[i];
					finalDirection[id]=scaleDirection[i][id];
				}
			}
	...
	private final void normalDirectionFromRecursiveRidgeFilter(float[] img, boolean[] mask, float[][] surfaces, float[] filter,byte[] direction,float[] product, int orientfactor) {
			...
			for (int x=2;x<nx-2;x++) for (int y=2;y<ny-2;y++) for (int z=2;z<nz-2;z++) {
				...
				if (mask[xyz] && !zeroNeighbor(img, mask, x,y,z,2)) {
					// find the surface normal direction
					int s0=0;
					for (int s=1;s<nsurf;s++) {
						if (Numerics.abs(surfaces[s][xyz])<Numerics.abs(surfaces[s0][xyz])) s0=s;
					}
					normal[X] = 0.5f*(surfaces[s0][Ngb.neighborIndex(Ngb.pX, xyz, nx,ny,nz)]-surfaces[s0][Ngb.neighborIndex(Ngb.mX, xyz, nx,ny,nz)]);
					normal[Y] = 0.5f*(surfaces[s0][Ngb.neighborIndex(Ngb.pY, xyz, nx,ny,nz)]-surfaces[s0][Ngb.neighborIndex(Ngb.mY, xyz, nx,ny,nz)]);
					normal[Z] = 0.5f*(surfaces[s0][Ngb.neighborIndex(Ngb.pZ, xyz, nx,ny,nz)]-surfaces[s0][Ngb.neighborIndex(Ngb.mZ, xyz, nx,ny,nz)]);
					
					// check for zero-valued neighbors as well
					normalMinmaxplaneScore(image, normal, planescore, planedir, x,y,z, 13, orientfactor);
					// sign issue: remove all that have different sign, keep global sign
					float linescore = parallelMinmaxlineScore(image, normal, planedir, linedir, x,y,z, 4, orientfactor);
					if (planescore[x][y][z]*linescore>0) {
						...
						product[xyz] = Numerics.abs(directionProduct(direction[xyz], normal, orientfactor));
						if (filter[xyz]<0) { filter[xyz]=0; direction[xyz] = -1; product[xyz]=0; }
					} else {
						filter[xyz] = 0.0f;
						direction[xyz] = -1;
						product[xyz] = 0.0f;
	...
	void normalMinmaxplaneScore(float[][][] image, float[] normal, float[][][] plane, byte[][][] dir, int x, int y, int z, int dmax, int orientfactor) {
		float maxgrad = 0.0f; 
		float minval = 0.0f; 
		float sign = 0.0f;
		byte direction = -1;
		for (byte d=0;d<dmax;d++) {
			...
			// modulate by orthogonality to the given normal
			float factor = (1.0f-Numerics.abs(directionProduct(d, normal, orientfactor)));
			
			if (factor*(val1*val1+val2*val2)>maxgrad) {
				maxgrad = factor*(val1*val1+val2*val2);
	...
	float parallelMinmaxlineScore(float[][][] image, float[] normal, byte[][][] planedir, byte[][][] linedir, int x, int y, int z, int dmax, int orientfactor) {
		float maxgrad = 0.0f; 
		float minval = 0.0f; 
		float sign = 0.0f;
		byte direction = -1;
		...
		if (planedir[x][y][z]==0) {
			for (byte d=0;d<dmax;d++) {
				...
				// modulate by parallelism to the given normal
				float factor = Numerics.abs(directionProduct(direction, normal, orientfactor));
				// find the strongest gradient direction, then estimate the corresponding filter response
				if (factor*(val1*val1+val2*val2)>maxgrad) {
					maxgrad = factor*(val1*val1+val2*val2);
					if (val1*val1<val2*val2) minval = val1;
	...
	private final float directionProduct(int dir, float[] normal, int orientfactor) {
		float dv=0.0f;
			
			 if (dir==X) dv = normal[0];
		else if (dir==Y) dv = normal[1];
		else if (dir==Z) dv = normal[2];
		else if (dir==XpY) dv = normal[0]/SQRT2+normal[1]/SQRT2;
		else if (dir==YpZ) dv = normal[1]/SQRT2+normal[2]/SQRT2;
		else if (dir==ZpX) dv = normal[2]/SQRT2+normal[0]/SQRT2;
		else if (dir==XmY) dv = normal[0]/SQRT2-normal[1]/SQRT2;
		else if (dir==YmZ) dv = normal[1]/SQRT2-normal[2]/SQRT2;
		else if (dir==ZmX) dv = normal[2]/SQRT2-normal[0]/SQRT2;
		else if (dir==XpYpZ) dv = normal[0]/SQRT3+normal[1]/SQRT3+normal[2]/SQRT3;
		else if (dir==XmYpZ) dv = normal[0]/SQRT3-normal[1]/SQRT3+normal[2]/SQRT3;
		else if (dir==XpYmZ) dv = normal[0]/SQRT3+normal[1]/SQRT3-normal[2]/SQRT3;
		else if (dir==XmYmZ) dv = normal[0]/SQRT3-normal[1]/SQRT3-normal[2]/SQRT3;
		else if (dir==mX) dv = -normal[0];
		else if (dir==mY) dv = -normal[1];
		else if (dir==mZ) dv = -normal[2];
		else if (dir==mXpY) dv = -(normal[0]/SQRT2+normal[1]/SQRT2);
		else if (dir==mYpZ) dv = -(normal[1]/SQRT2+normal[2]/SQRT2);
		else if (dir==mZpX) dv = -(normal[2]/SQRT2+normal[0]/SQRT2);
		else if (dir==mXmY) dv = -(normal[0]/SQRT2-normal[1]/SQRT2);
		else if (dir==mYmZ) dv = -(normal[1]/SQRT2-normal[2]/SQRT2);
		else if (dir==mZmX) dv = -(normal[2]/SQRT2-normal[0]/SQRT2);
		else if (dir==mXpYpZ) dv = -(normal[0]/SQRT3+normal[1]/SQRT3+normal[2]/SQRT3);
		else if (dir==mXmYpZ) dv = -(normal[0]/SQRT3-normal[1]/SQRT3+normal[2]/SQRT3);
		else if (dir==mXpYmZ) dv = -(normal[0]/SQRT3+normal[1]/SQRT3-normal[2]/SQRT3);
		else if (dir==mXmYmZ) dv = -(normal[0]/SQRT3-normal[1]/SQRT3-normal[2]/SQRT3);

		if (orientfactor==1) return dv;
		else if (orientfactor==2) return dv*dv;
		else if (orientfactor==3) return dv*dv*dv;
		else if (orientfactor==4) return dv*dv*dv*dv;
		else if (orientfactor==5) return dv*dv*dv*dv*dv;
		else if (orientfactor==6) return dv*dv*dv*dv*dv*dv;
		else return dv;
	}	
