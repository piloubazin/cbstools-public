...
import de.mpg.cbs.core.segmentation.SegmentationLesionExtraction2;

/*
 * @author Pierre-Louis Bazin
 */
public class JistSegmentationLesionExtraction2 extends ProcessingAlgorithm {
	...
	private ParamFloat		wmdistanceParam;
	...
	private SegmentationLesionExtraction2 algorithm;
	
	protected void createInputParameters(ParamCollection inputParams) {
		
		inputParams.add(probaImage = new ParamVolume("Probability Image"));
		inputParams.add(wmImage = new ParamVolume("WM Region Image"));
		inputParams.add(gmImage = new ParamVolume("GM Background Image"));
		inputParams.add(csfImage = new ParamVolume("CSF Background Image"));
		
		inputParams.add(wmdistanceParam = new ParamFloat("WM boundary partial voluming distance (voxels)",0.0f,100.0f,0.5f));
		...
		inputParams.add(minprobaThresholdParam = new ParamFloat("Probability minimum threshold (to include in analysis)",0.0f,1.0f,0.84f));
		inputParams.add(maxprobaThresholdParam = new ParamFloat("Probability maximum threshold (to label as local maxima)",0.0f,1.0f,0.97f));
		inputParams.add(minSizeParam = new ParamFloat("Small lesion size (voxels)",0.0f,10000.0f,4.0f));

		algorithm = new SegmentationLesionExtraction2();
	...
	protected void execute(CalculationMonitor monitor){
		...
		algorithm.setResolutions(Interface.getResolutions(probaImage));
		
		algorithm.setProbaImage(Interface.getFloatImage3D(probaImage));
		algorithm.setWMRegionImage(Interface.getFloatImage3D(wmImage));
		algorithm.setGMRegionImage(Interface.getFloatImage3D(gmImage));
		algorithm.setCSFRegionImage(Interface.getFloatImage3D(csfImage));
		
		algorithm.setWMPartialVolumingDistance(wmdistanceParam.getValue().floatValue());
