...
public class BrainDefineMultiRegionPriors {
	...
	public final float[] getRegionProbability() { return pvRegionImage; }
	...
	public final void execute(){
		...
		System.out.println("Defined region: "+regionParam);
		
		if (regionParam.equals("inter-ventricular")) {
			regionName = "_iv";
			...
			for (int xyz=0;xyz<nxyz;xyz++) {
				...
				if (lvlvenL[xyz]<2*maxdist && lvlvenR[xyz]<2*maxdist)
					pvRegionImage[xyz] = Numerics.bounded(1.0f - Numerics.max(lvlvenL[xyz],lvlvenR[xyz])/(2.0f*maxdist), 0.0f, 1.0f);
				else
					pvRegionImage[xyz] = 0.0f;
			}
		} else if (regionParam.equals("ventricle-horns")) {
			regionName = "_vh";
