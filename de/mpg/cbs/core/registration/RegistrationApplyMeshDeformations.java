 ...
 */
public class RegistrationApplyMeshDeformations {
	...
	// create inputs
	public final void setMeshPointsToDeform(float[] val) { sourcePoints = val; }
	...
	public final float[] getDeformedMeshPoints() { return deformedPoints; }
	...
	public void execute() {
        ...
        // new image
        System.out.println("deform surface mesh");
        int npt = sourcePoints.length/3;
        ...
        if (padOption.equals("min") || padOption.equals("max")) {
            for (int p=0;p<npt;p++) for (int n=0;n<3;n++) {
                if (sourcePoints[3*p + n]<min[n]) min[n] = sourcePoints[3*p + n];
                if (sourcePoints[3*p + n]>max[n]) max[n] = sourcePoints[3*p + n];
            }
        }
       
        deformedPoints = new float[3*npt];
        
        if (interpOption.equals("nearest")) {
            for (int p=0;p<npt;p++) {
                for (int n=0;n<3;n++) {
                    if (padOption.equals("closest"))
                        deformedPoints[3*p + n] = ImageInterpolation.nearestNeighborClosestInterpolation(deformation, sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3);
                     else if (padOption.equals("zero"))
                        deformedPoints[3*p + n] = ImageInterpolation.nearestNeighborInterpolation(deformation, 0.0f, sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3);
                     else if (padOption.equals("min"))
                        deformedPoints[3*p + n] = ImageInterpolation.nearestNeighborInterpolation(deformation, min[n], sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3);
                    else if (padOption.equals("max"))
                        deformedPoints[3*p + n] = ImageInterpolation.nearestNeighborInterpolation(deformation, max[n], sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3); 
                }
            }
            ...
            for (int p=0;p<npt;p++) {
                for (int n=0;n<3;n++) {
                    if (padOption.equals("closest"))
                        deformedPoints[3*p + n] = ImageInterpolation.linearClosestInterpolation(deformation, sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3);
                    else if (padOption.equals("zero"))
                        deformedPoints[3*p + n] = ImageInterpolation.linearInterpolation(deformation, 0.0f, sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3);
                    else if (padOption.equals("min"))
                        deformedPoints[3*p + n] = ImageInterpolation.linearInterpolation(deformation, min[n], sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3);
                    else if (padOption.equals("max"))
                        deformedPoints[3*p + n] = ImageInterpolation.linearInterpolation(deformation, max[n], sourcePoints[3*p+X], sourcePoints[3*p+Y], sourcePoints[3*p+Z], n, nrx, nry, nrz, 3);
                }
            }
        }
        sourcePoints = null;
