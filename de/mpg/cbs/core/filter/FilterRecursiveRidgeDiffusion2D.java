...
public class FilterRecursiveRidgeDiffusion2D {
	...
	private String filterParam = "1D";
	public static final String[] filterTypes = {"1D","0D"};
	...
	public final String getLabel() { return "Recursive Ridge Diffusion 2D"; }
	public final String getName() { return "RecursiveRidgeDiffusion2D"; }
	...
	public void execute(){
		...
		if (orientationParam.equals("parallel") || orientationParam.equals("orthogonal")) {
			...
			for (int xyz=0;xyz<nxyz;xyz++) if (mask[xyz]) {
				...
				double norm = grad[X]*grad[X]+grad[Y]*grad[Y];
				if (norm>0) {
				...
				double angle = 2.0*FastMath.acos(grad[X]*vec[X]+grad[Y]*vec[Y])/FastMath.PI;
		...
		*/
		...
		if (propagationParam.equals("SUR")) {
			propag = spatialUncertaintyRelaxation1D(proba, maxdirection, ngbParam, simscaleParam, difffactorParam, iterParam);
			...
			propag = beliefPropagation1D(proba, maxdirection, ngbParam, simscaleParam, iterParam, maxdiffParam);
		} else if  (propagationParam.equals("flooding")) {
			propag = probabilisticDiffusion1D(proba, maxdirection, ngbParam, maxdiffParam, simscaleParam, difffactorParam, iterParam);
		} else if  (propagationParam.equals("growing")) {
			propag = regionGrowing1D(proba, maxdirection, ngbParam, maxdiffParam, simscaleParam, difffactorParam, iterParam);
		} else if  (propagationParam.equals("labeling")) {
			propag = regionLabeling1D(proba, maxdirection, ngbParam, maxdiffParam, simscaleParam, difffactorParam, iterParam);
		} else {
		    propag = proba;
		}
		...
		*/
			...
			if (propag[xyz]>0.0f) {
				...
				for (int i=-nsc;i<=nsc;i++) for (int j=-nsc;j<=nsc;j++) {
					if (x+i>=0 && x+i<nx && y+j>=0 && y+j<ny) {
						...
						pvol[xyz+i+j*nx] = Numerics.max(pvol[xyz+i+j*nx], ratio*pvol[xyz]);
					}
				}
			}
		}
		...
		float[] result4 = new float[nxyz*2];
		for (int xyz=0;xyz<nxyz;xyz++) {
	...
	private final void directionFromRecursiveRidgeFilter0D(float[] img, boolean[] mask, float[] filter, byte[] direction, boolean unidirectional) {
        ...
        for (int x=2;x<nx-2;x++) for (int y=2;y<ny-2;y++) for (int z=0;z<nz;z++) {
            ...
            if (mask[xyz] && !zeroNeighbor(img, mask, x,y,z,2)) {
                ...
                if (pointscore<0 && unidirectional) { 
                    filter[xyz]=0; direction[xyz] = -1; 
                }
            }
        }
        linescore = null;
	...
	private final float[] spatialExpansion0D(float[] proba, int[] scale, float[] intens) {
		...
		for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++) {
			...
			if (proba[xyz]>0.0f && scale[xyz]>0) {
				...
				for (int i=-dim;i<=dim;i++) for (int j=-dim;j<=dim;j++) {
					if (x+i>=0 && x+i<nx && y+j>=0 && y+j<ny) {
					    float ratio = (float)FastMath.exp( - 0.5f*(i*i+j*j)/(scale[xyz]*scale[xyz]) );
					    float imgratio = intens[xyz+i+j*nx]*intens[xyz];
						propag[xyz+i+j*nx] = Numerics.max(propag[xyz+i+j*nx], imgratio*ratio*proba[xyz]);
					}
				}
			}
	...
	private final float[] spatialUncertaintyRelaxation1D(float[] proba, byte[] dir, int ngbParam, float scale, float factor, int iterParam) {
		...
		float[] ngbweight = new float[NC2];
		float[][] imgweight = new float[nx*ny*nz][NC2];  
		...
		//for (int t=0;t<iterParam && maxdiffParam>0.1f;t++) {
			...
			for (int x=1;x<nx-1;x++) for (int y=1;y<ny-1;y++) for (int z=0;z<nz;z++) {
				...
				for (int i=-1;i<=1 && !diffmask[xyz];i++) for (int j=-1;j<=1 && !diffmask[xyz];j++) {
					if (prevproba[xyz+i+j*nx]>0 && prevproba[xyz+i+j*nx]<1) diffmask[xyz] = true;
				}
			}
