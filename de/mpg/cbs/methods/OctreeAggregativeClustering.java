...
/**
 *
 *  This algorithm builds a hierarchical clustering of image data with an octree structure
 *	followed by a fully hierarchical clustering
 *
 *
 *	@version    March 2015
...
public class OctreeAggregativeClustering {
	...
	private		byte		mode;
	private	static final	byte	GAUSSIAN = 10;
	 ...
	 *  constructor
	 */
	public OctreeClustering(OctreeCluster oct_, boolean[] msk_,
		...
		if (mod_.equals("gaussian")) 			mode = GAUSSIAN;
	...
	public final void buildOctreeClusters() {
		
		int[] octreescale = new int[nx*ny*nz];
		float[]	octreeweight = new float[nx*ny*nz];
		float[] octreedelta = new float[nx*ny*nz];
		...
		for (int x=0;x<nx;x+=step) for (int y=0;y<ny;y+=step) for (int z=0;z<nz;z+step) {
			int xyz = x + nx*y + nx*ny*z;
			if (mask[xyz]) {
			}
		}
	...
	private final float associationWeight(int id1, int id2) {
		if (mode==GAUSSIAN) return gaussDistanceWeight(id1, id2);
		return 0.5f;
	}
	...
	private final float dataDistance(int id1, int id2) {
		if (mode==GAUSSIAN) return gaussDistance(id1, id2);
		return 0.5f;
	}
