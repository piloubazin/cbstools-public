...
/**
 *
 *  This algorithm uses the classical GDM framework to evolve a surface
 *	according to volume-preserving forces for cortical layering.
 ...
 *	@author     Miriam Waehnert
 *		
 *
 */
 
public class VolumetricLayeringGdm {
	...
	private 	float[] 		outlevelset;  		// outerlevelset
	...
	private float[][]   volumein, volumeout; 
	private float[]		lvlratio;
	
	private float		curvscale;
	
	// model for determining the distance to reach
	private	byte				model = 0;
	private static final byte	DISTANCE = 1;
	private static final byte	VOLUME = 2;
	private static final byte	VOLUME2 = 3;
	
	// model for determining the direction of propagation
	private	byte				layeringDir = 0;
	private static final byte	INWARD = 1;
	private static final byte	OUTWARD = 2;
	...
	// for debug
	private float[]   array_Ain;
	private float[]	  array_Aout;
	
	private float[]	  array_fBisektLastIt;
	private float[]   array_ratioLastIt;
	private float[]   array_ratio;
	private float[]   array_fraction;
	private float[]   array_fBisekt;
	...
	public VolumetricLayeringGdm(float[] lvlsetin_, float[] lvlsetout_, String modelType_, String dirType_,
		...
		if (modelType_.equals("distance-preserving")) model = DISTANCE;
		else if (modelType_.equals("volume-preserving")) model = VOLUME;
		else if (modelType_.equals("volume-preserving2")) model = VOLUME2;
		else model = DISTANCE;
		
		if (dirType_.equals("inward")) layeringDir = INWARD;
		else if (dirType_.equals("outward")) layeringDir = OUTWARD;
		...
		if (model == VOLUME || model == VOLUME2) {
			volumein = cin_;
			volumeout = cout_;
			
			lvlratio = new float[nx*ny*nz];
			
			if (debug) {
				array_Ain = new float[nx*ny*nz];
				array_Aout = new float[nx*ny*nz];
				array_ratioLastIt = new float[nx*ny*nz];
				array_ratio = new float[nx*ny*nz];
				array_fraction = new float[nx*ny*nz];
				array_fBisekt = new float[nx*ny*nz];
			}	
		}			
		...
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) {
			// gdm functions
			if (layeringDir==OUTWARD) {
				levelset[xyz] = inlevelset[xyz];  
			} else if (layeringDir==INWARD) {
				levelset[xyz] = outlevelset[xyz];  
			}
	...
	public final float[] exportAinArray(){
		System.out.println( "exporting Ain array");
		float[] res = new float[nx*ny*nz];
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) res[xyz] = array_Ain[xyz];
		return res;
	}

	public final float[] exportAoutArray(){
		System.out.println( "exporting Aout array");
		float[] res = new float[nx*ny*nz];
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) res[xyz] = array_Aout[xyz];
		return res;
	}
	    
	public final float[] exportLvlratio() {
		float[] res = new float[nx*ny*nz];
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) res[xyz] = lvlratio[xyz];
		return res;
	}
    ...
    public final void setFraction(float frac) { fraction = frac; }
    ...
    */
    ...
    private final double levelsetForces(int xyz) {
		...
		// compute a "target" levelset
		double target_temp = 0;
		double target = 0;
		if (model==DISTANCE) target = (1.0-fraction)*inlevelset[xyz] + fraction*outlevelset[xyz];
		else if (model==VOLUME || model==VOLUME2) {
			target = (1.0-lvlratio[xyz])*inlevelset[xyz] + lvlratio[xyz]*outlevelset[xyz];
		}
		
		balloonforce = -Numerics.bounded( target - levelset[xyz], -1.0, 1.0);
    ...
    public void computeVolumetricRatio(float kscale, boolean[] cortex){
    	
    	for (int xyz = 0; xyz<nx*ny*nz; xyz++){

    		if (cortex[xyz]) {
    			float ki1 = Numerics.abs(volumein[1][xyz]);
    			float ki2 = Numerics.abs(volumein[2][xyz]);
    			float ko1 = Numerics.abs(volumeout[1][xyz]);
    			float ko2 = Numerics.abs(volumeout[2][xyz]);

    			float d = volumein[0][xyz];
    			float length_i1 = 10.0f;
    			float length_i2 = 10.0f;
    			float length_o1 = 10.0f;
    			float length_o2 = 10.0f;
    			float Ao = -1.0f;
    			float Ai = -1.0f;

    			if (ki1>ko1){
    				length_i1 = Numerics.bounded((float)(1/(1+0.5*d*curvscale*ki1/rx)), 1e-6f, 1e6f); 
    				length_o1 = Numerics.bounded((float)(1/(1-0.5*d*curvscale*ko1/rx)), 1e-6f, 1e6f);			
    			}
    			else if (ko1>ki1){
    				length_i1 = Numerics.bounded((float)(1/(1-0.5*d*curvscale*ki1/rx)), 1e-6f, 1e6f);
    				length_o1 = Numerics.bounded((float)(1/(1+0.5*d*curvscale*ko1/rx)), 1e-6f, 1e6f);
    			...
    			if (ki2>ko2){
    				length_i2 = Numerics.bounded((float)(1/(1+0.5*d*curvscale*ki2/rx)), 1e-6f, 1e6f); 
    				length_o2 = Numerics.bounded((float)(1/(1-0.5*d*curvscale*ko2/rx)), 1e-6f, 1e6f);			
    			}
    			else if (ko2>ki2){
    				length_i2 = Numerics.bounded((float)(1/(1-0.5*d*curvscale*ki2/rx)), 1e-6f, 1e6f);
    				length_o2 = Numerics.bounded((float)(1/(1+0.5*d*curvscale*ko2/rx)), 1e-6f, 1e6f);
    			...
    			Ai = length_i1*length_i2;
    			Ao = length_o1*length_o2;
    			...
    			if (debug) {
    	       	...
    	       	float ratio;
    	       	if (Numerics.abs(Ao-Ai)< 1e-8f){
    	       		ratio = fraction;
    	       	}
    	       	else{
    	       		/*
    	       		if (layeringDir==OUTWARD) {
    	       			// formula for outward layering: 
    	       			ratio = 1.0f/(Ao-Ai)*(-Ai+(float)FastMath.sqrt(fraction*Ao*Ao + Ai*Ai*(1.0f-fraction)));
    	       		} else if (layeringDir==INWARD) {
						// formula for inward layering: 
						ratio = 1.0f - (1.0f/(Ao-Ai)*(-Ai+(float)FastMath.sqrt((1.0f-fraction)*Ao*Ao + Ai*Ai*fraction)));
					} else {
						ratio = fraction;
					}
					*/
					// no need to separate inward and outward, as the targets are the same 
					// (only the starting point and the ordering of the layering change)
					ratio = 1.0f/(Ao-Ai)*(-Ai+(float)FastMath.sqrt(fraction*Ao*Ao + Ai*Ai*(1.0f-fraction)));
    	       	}
    	       	
    	    	lvlratio[xyz] = ratio;
    	    } else {
    	    	lvlratio[xyz] = fraction;
    	    }
    	}
    	
  		float[][] kernel = ImageFilters.separableGaussianKernel(kscale, kscale, kscale);		
  		lvlratio = ImageFilters.separableMaskedConvolution(lvlratio, cortex, nx, ny, nz, kernel);
  		return;	
    }
     
   public void computeVolumetricRatio2(float kscale, boolean[] cortex){
    	
    	for (int xyz = 0; xyz<nx*ny*nz; xyz++){

    		if (cortex[xyz]) {
    			float Ai = volumein[0][xyz];
    			float Ao = volumeout[0][xyz];
