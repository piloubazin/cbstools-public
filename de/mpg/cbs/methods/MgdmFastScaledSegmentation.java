...
public class MgdmFastScaledSegmentation {
	 ...
	 */
	public MgdmFastScaledSegmentation(float[][] img_, String[] mod_, float[] rng_, int nc_,
		...
		// init segmentation from atlas or previous segmentation
		if (init_!=null) {
			for (int x=0;x<nsx;x++) for (int y=0;y<nsy;y++) for (int z=0;z<nsz;z++) {
				int xyz = x + nsx*y + nsx*nsy*z;
				
				int init = ImageInterpolation.nearestNeighborInterpolation(init_,(byte)1,x*scale,y*scale,z*scale,nix,niy,niz);
				byte nlb = EMPTY;
