...
public class MgdmFastSegmentation2 {
	 ...
	 */
	public MgdmFastSegmentation2(float[][] img_, String[] mod_, float[] rng_, int nc_,
		...
		this(img_, mod_, rng_, nc_, nix_, niy_, niz_, rix_, riy_, riz_, atlas_, initatlas_, initscaled_, nmgdm_, ngain_, fw_, sw_, dw_, gd_, connectivityType_, null);
	}
	...
	public MgdmFastSegmentation2(float[][] img_, String[] mod_, float[] rng_, int nc_,
		...
		try {
			...
			segobjlabels = new byte[nix*niy*niz];	
		...
		if (initscaled_!=null) {
			for (int x=0;x<nix;x++) for (int y=0;y<niy;y++) for (int z=0;z<niz;z++) {
				...
				segmentation[xyz] = initscaled_[xyz];
				
				// remove the -1 labels, if any (replace by "background")
				if (segmentation[xyz]<0) segmentation[xyz]=0;
				
				if (segmentation[xyz]>-1) segobjlabels[xyz] = atlas.getLabels()[segmentation[xyz]];
				else segobjlabels[xyz] = -1;
	...
	public final void setBestGainFunctions2(float[][] gain, byte[][] label) { 
		bestgain = gain; 
		bestlabel = label; 
    ...
    public final float[] exportBestGainFunctions1D(int n0, int nmax, boolean rescale) {
    	float[] result = new float[nix*niy*niz*(nmax-n0+1)];
    	
    	for (int x=0;x<nix;x++) for (int y=0;y<niy;y++) for (int z=0;z<niz;z++) for (int n=n0;n<=nmax;n++) {
    		int xyzn = x+nix*y+nix*niy*z+nix*niy*niz*n;
    		if (rescale) result[xyzn] = 0.5f+0.5f*bestgain[n][x+nix*y+nix*niy*z];
     		else result[xyzn] = bestgain[n][x+nix*y+nix*niy*z];
		}
		return result;
    }
    ...
    public final byte[][][] exportBestGainByte() {
    	byte[][][] seg = new byte[nix][niy][niz];
    	int id;
    	for (int x=0; x<nix; x++) for (int y=0; y<niy; y++) for (int z=0; z<niz; z++) {
    		id = bestlabel[0][x+nix*y+nix*niy*z]; 
    ...
    public final byte[] exportBestGainLabelsByte1D(int n0, int nmax) {
    	byte[] seg = new byte[nix*niy*niz*(nmax-n0+1)];
    	int id;
    	for (int x=0; x<nix; x++) for (int y=0; y<niy; y++) for (int z=0; z<niz; z++) for (int n=n0;n<=nmax;n++) {
    		int xyzn = x+nix*y+nix*niy*z+nix*niy*niz*n;
    		id = bestlabel[n][x+nix*y+nix*niy*z];
    		if (id>-1) seg[xyzn] = (byte)atlas.getLabels()[id];
    		else seg[xyzn] = -1;
    	}
		return seg;
    }
    public final byte[][][][] exportBestGainsByte(int n0, int nmax) {
    	byte[][][][] seg = new byte[nix][niy][niz][nmax-n0+1];
    	int id;
    	for (int x=0; x<nix; x++) for (int y=0; y<niy; y++) for (int z=0; z<niz; z++) for (int n=n0;n<=nmax;n++) {
    		id = bestlabel[n][x+nix*y+nix*niy*z];
    ...
    public final byte[] exportBestGainsByte1D(int n0, int nmax) {
    	byte[] seg = new byte[nix*niy*niz*(nmax-n0+1)];
    	int id;
    	for (int x=0; x<nix; x++) for (int y=0; y<niy; y++) for (int z=0; z<niz; z++) for (int n=n0;n<=nmax;n++) {
    		int xyzn = x+nix*y+nix*niy*z+nix*niy*niz*n;
    ...
    */
    ...
    private final boolean homeomorphicLabeling(int xyz, byte lb) {
		...
		for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
			...
			if (segobjlabels[xyz+i+j*nix+l*nix*niy]==objLabel[lb]) {
		...
		for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
			...
			if (segobjlabels[xyz+i+j*nix+l*nix*niy]==segobjlabels[xyz]) {
				obj[1+i][1+j][1+l] = true;
			} else {
				obj[1+i][1+j][1+l] = false;
			}
		...
		for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
			...
			if ( (i*i+j*j+l*l>0) 
				&& (segobjlabels[xyz+i+j*nix+l*nix*niy]!=objLabel[lb]) 
				&& (segobjlabels[xyz+i+j*nix+l*nix*niy]!=segobjlabels[xyz]) ) {
				found = false;
				for (int n=0;n<Nconfiguration;n++) 
					//if (segmentation[xyz+i+j*nix+l*nix*niy]==lbs[n]) { found = true; break; }
					if (segobjlabels[xyz+i+j*nix+l*nix*niy]==objLabel[lbs[n]]) { found = true; break; }
		...
		for (int n=0;n<Nconfiguration;n++) {
			...
			for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
				...
				if ( (segobjlabels[xyz+i+j*nix+l*nix*niy]==segobjlabels[xyz])
					|| (segobjlabels[xyz+i+j*nix+l*nix*niy]==objLabel[lbs[n]]) ) {
					obj[1+i][1+j][1+l] = true;
				} else {
					obj[1+i][1+j][1+l] = false;
				}
		...
		for (int n=0;n<Nconfiguration;n++) {
			for (int m=n+1;m<Nconfiguration;m++) {
				...
				for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
					...
					if ( (segobjlabels[xyz+i+j*nix+l*nix*niy]==segobjlabels[xyz])
						|| (segobjlabels[xyz+i+j*nix+l*nix*niy]==objLabel[lbs[n]])
						|| (segobjlabels[xyz+i+j*nix+l*nix*niy]==objLabel[lbs[m]]) ) {
						obj[1+i][1+j][1+l] = true;
					} else {
						obj[1+i][1+j][1+l] = false;
					}
