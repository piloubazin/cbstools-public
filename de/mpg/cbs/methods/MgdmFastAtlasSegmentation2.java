...
public class MgdmFastAtlasSegmentation2 {
	...
	private 	byte[] 			segobjlabels;   	// MGDM's segmentation (object labels -> multiple objects may have same label)
	...
	public MgdmFastAtlasSegmentation2(float[][] img_, String[] mod_, float[] rng_, int nc_,
		...
		try {
			...
			segobjlabels = new byte[nax*nay*naz];	
		...
		// init segmentation from atlas : keep the labels!
		for (int x=0;x<nax;x++) for (int y=0;y<nay;y++) for (int z=0;z<naz;z++) {
	...
	public final byte[] getSeglabelings() { return segobjlabels; }
    ...
    public final void computeAtlasBestGainFunction() {
    	...
    	for (int x=0;x<nix;x++) for (int y=0;y<niy;y++) for (int z=0;z<niz;z++) {
			...
			for (int n=0;n<nc;n++) {
				if (image[n][xyzi]!=0) isMasked = false;
			}
			...
			if (isMasked) {
				...
				for (int n=0;n<nobj;n++) {
					...
					for (int c=0;c<nc;c++) {
						...
						if (atlas.isNormalizedIntensityContrast(intensityId[c])) {
							contrast[c] = -1.0f;
							if  (image[c][xyzi]!=0) {
								for (int t=0;t<atlas.getMap(intensityId[c])[n].length;t+=3) {
									diff = Numerics.abs(image[c][xyzi]/imrange[c]-atlas.getMap(intensityId[c])[n][t])/atlas.getMap(intensityId[c])[n][t+1];
									// this check doesn't make sense; unless looking for uncertainty scores
									//if (atlas.getMap(intensityId[c])[n][t+2]>0)
									// this is over multiple peaks of the same distribution (-> find shortest distance)
									contrast[c] = Numerics.max(contrast[c], atlas.getMap(intensityId[c])[n][t+2]*(1.0f - diff*diff)/(1.0f + diff*diff) );
								}
							} else if (n==0) contrast[c] = 1.0f;
						} else if (atlas.isQuantitativeContrast(intensityId[c])) {
							contrast[c] = -1.0f;
							if  (image[c][xyzi]!=0) {
								for (int t=0;t<atlas.getMap(intensityId[c])[n].length;t+=3) {
									diff = Numerics.abs(image[c][xyzi]-atlas.getMap(intensityId[c])[n][t])/atlas.getMap(intensityId[c])[n][t+1];
									// this check doesn't make sense; unless looking for uncertainty scores
									//if (atlas.getMap(intensityId[c])[n][t+2]>0)
									// this is over multiple peaks of the same distribution (-> find shortest distance)
									contrast[c] = Numerics.max(contrast[c], atlas.getMap(intensityId[c])[n][t+2]*(1.0f - diff*diff)/(1.0f + diff*diff) );
					...
					filterproba = 0.0f;
					filterval = 0.0f;
					for (int c=0;c<nc;c++) if (atlas.isFilterContrast(intensityId[c])) {
						if (image[c][xyzi]>=0 && image[c][xyzi]<=1) {
							// first level
							filterproba = image[c][xyzi];
							filterval = atlas.getMap(intensityId[c])[n][0];
						} else if (image[c][xyzi]>=2 && image[c][xyzi]<=3) {
							// second level
							filterproba = image[c][xyzi]-2.0f;
							filterval = atlas.getMap(intensityId[c])[n][1];
						} else if (image[c][xyzi]>=4 && image[c][xyzi]<=5) {
							// third level
							filterproba = image[c][xyzi]-4.0f;
							filterval = atlas.getMap(intensityId[c])[n][2];
						}
					}
					pvproba = 0.0f;
					pvval = 0.0f;
					for (int c=0;c<nc;c++) if (atlas.isProbabilityContrast(intensityId[c])) {
						if (image[c][xyzi]<0) {
							pvproba = Numerics.min(1.0f, -image[c][xyzi]/imrange[c]);
							pvval = atlas.getMap(intensityId[c])[n][0];
						} else {
							pvproba = Numerics.min(1.0f, image[c][xyzi]/imrange[c]);
					...
					for (int c=0;c<nc;c++) if (atlas.isFilterContrast(intensityId[c])) {
						if (filterval!=0) intens[n] = filterproba*filterval + (1.0f-filterproba)*intens[n];
					}
					for (int c=0;c<nc;c++) if (atlas.isProbabilityContrast(intensityId[c])) {
    ...
    */
						...
						if (narrowband.functions[lb][n]<0) {
							...
							if (newlb!=EMPTY) {
								...
								if (lb==0) {
									...
									segobjlabels[xyz] = objLabel[mgdmlabels[newlb][xyz]];
    ...
    private final boolean homeomorphicLabeling(int xyz, byte lb) {
		...
		for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
			...
			if (segobjlabels[xyz+i+j*nax+l*nax*nay]==objLabel[lb]) {
		...
		for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
			...
			if (segobjlabels[xyz+i+j*nax+l*nax*nay]==segobjlabels[xyz]) {
				obj[1+i][1+j][1+l] = true;
			} else {
				obj[1+i][1+j][1+l] = false;
			}
		...
		for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
			...
			if ( (i*i+j*j+l*l>0) 
				&& (segobjlabels[xyz+i+j*nax+l*nax*nay]!=objLabel[lb]) 
				&& (segobjlabels[xyz+i+j*nax+l*nax*nay]!=segobjlabels[xyz]) ) {
				found = false;
				for (int n=0;n<Nconfiguration;n++) 
					//if (segmentation[xyz+i+j*nax+l*nax*nay]==lbs[n]) { found = true; break; }
					if (segobjlabels[xyz+i+j*nax+l*nax*nay]==objLabel[lbs[n]]) { found = true; break; }
		...
		for (int n=0;n<Nconfiguration;n++) {
			...
			for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
				...
				if ( (segobjlabels[xyz+i+j*nax+l*nax*nay]==segobjlabels[xyz])
					|| (segobjlabels[xyz+i+j*nax+l*nax*nay]==objLabel[lbs[n]]) ) {
					obj[1+i][1+j][1+l] = true;
				} else {
					obj[1+i][1+j][1+l] = false;
				}
		...
		for (int n=0;n<Nconfiguration;n++) {
			for (int m=n+1;m<Nconfiguration;m++) {
				...
				for (int i=-1;i<=1;i++) for (int j=-1;j<=1;j++) for (int l=-1;l<=1;l++) {
					...
					if ( (segobjlabels[xyz+i+j*nax+l*nax*nay]==segobjlabels[xyz])
						|| (segobjlabels[xyz+i+j*nax+l*nax*nay]==objLabel[lbs[n]])
						|| (segobjlabels[xyz+i+j*nax+l*nax*nay]==objLabel[lbs[m]]) ) {
						obj[1+i][1+j][1+l] = true;
					} else {
						obj[1+i][1+j][1+l] = false;
					}
