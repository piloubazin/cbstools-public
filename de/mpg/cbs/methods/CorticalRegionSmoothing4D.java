...
public class CorticalRegionSmoothing4D {
	...
	private static int			nd;					// 4th dimension
	private		float			sigma;
	...
	// new constructor
	public CorticalRegionSmoothing4D(float[][] data_, float fwhm_, int nx_, int ny_, int nz_, int nd_,
										float rx_, float ry_, float rz_, boolean[] msk_) {
	...
	public final void dilateFromPoint(float[] smoothed, int xyz0) {
		
		float dist = 0.0f;
        double[] sdata = new double[nd];
		...
		for (int d=0;d<=dmax;d++) {
			...
			for (int xyz = boundary.nextSetBit(0); xyz >= 0; xyz = boundary.nextSetBit(xyz+1)) {
				...
				for (int n=0;n<nd;n++)
					sdata[n] += gaussWeight*data[n][xyz];
