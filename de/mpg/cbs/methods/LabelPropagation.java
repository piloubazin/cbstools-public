...
/**
 *
 *  This algorithm handles basic manipulations of multi-labels maps
 *
 *	@version    June 2013
...
public class LabelPropagation {
	...
	private 	byte[] 			labels;   		// MGDM's label maps
	 ...
	 */
	public LabelPropagation(byte[] labels_, int nx_, int ny_, int nz_, float rx_, float ry_, float rz_,
								int nlb_, boolean skip_, float dist_) {
		...
		for (int x=0; x<nx; x++) for (int y=0; y<ny; y++) for (int z = 0; z<nz; z++) {
			...
			if (skipZero && labels[x+nx*y+nx*ny*z]==0) mask[x+nx*y+nx*ny*z] = false;
		}
		// init segmentation (from atlas if null)
		for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++) {
			int xyz = x + nx*y + nx*ny*z;
			
			segmentation[xyz] = labels[xyz];
			counter[xyz] = 0;			
		}
    ...
    public final float[][] reconstructedLevelSets() {
    	...
    	for (int n=0;n<nobj;n++) {
    		for (int xyz=0; xyz<nx*ny*nz; xyz++) {
    			...
    			for (int l=0;l<nlb && mgdmlabels[l][xyz]!=n;l++) {
    ...
    public final float[][] reconstructedExactLevelSets() {
    	...
    	for (int n=0;n<nobj;n++) {
    		for (int xyz=0; xyz<nx*ny*nz; xyz++) {
    			...
    			if (max==nlb) levelsets[n][xyz] = UNKNOWN;
    		}
    	}
    	return levelsets;
    }
    ...
    public final int[][] reconstructedLabels() {
    	int[][] labels = new int[nlb][nx*ny*nz];
    	for (int n=0;n<nlb;n++) {
    		for (int xyz=0; xyz<nx*ny*nz; xyz++) {
    ...
    public final byte[][][][] exportLabels() {
    	byte[][][][] seg = new byte[nx][ny][nz][nlb+1];
    	for (int x=0; x<nx; x++) for (int y=0; y<ny; y++) for (int z=0; z<nz; z++) for (int n=0;n<=nlb;n++) {
    		int xyz = x + nx*y + nx*ny*z;
    		if (mgdmlabels[n][xyz]>-1) {
    ...
    public final float[][][][] exportFunctions() {
    	float[][][][] fun = new float[nx][ny][nz][nlb];
    	for (int x=0; x<nx; x++) for (int y=0; y<ny; y++) for (int z=0; z<nz; z++) for (int n=0;n<nlb;n++) {
    		int xyz = x + nx*y + nx*ny*z;
