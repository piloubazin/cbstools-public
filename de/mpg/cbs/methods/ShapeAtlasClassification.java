...
public class ShapeAtlasClassification {
	...
	private		float[]			imrange;			// image range
	 ...
	 *  constructor
	 */
	public ShapeAtlasClassification(float[][] img_, String[] mod_, int nc_,
    ...
    final public void initialAtlasTissueCentroids() {
		...
		for (int c=0;c<nc;c++) for (int t=0;t<nt;t++) tissues[c][t] = 0.0f;
    ...
    final public void computeMemberships() {
        ...
        for (int x=0;x<nax;x++) for (int y=0;y<nay;y++) for (int z=0;z<naz;z++) {
			...
			for (int c=0;c<nc;c++)  if (atlas.isIntensityContrast(atlas.contrastId(modality[c]))) {
				...
				if (processed[BG]) {
					num[BG] += (img-centroids[c][BG])/(0.01f*imrange[c])*(img-centroids[c][BG])/(0.01f*imrange[c]);
				}
				for (int k=1;k<nclass;k++) if (processed[k]) {
					num[k] += (img-centroids[c][k])/imrange[c]*(img-centroids[c][k])/imrange[c];
				}
			}
    ...
    final public void estimateIntensityTransform() {
    	...
    	for (int c=0;c<nc;c++) if (atlas.isIntensityContrast(atlas.contrastId(modality[c]))) {
			...
			float[][] 	tissuepriors = new float[3][3];
			float[] 	imagevalues = new float[3];
			
			// compute a simple quadratic fit?
			for (int k=1;k<nclass;k++) if (processed[k]) {
				tissuepriors[k-1][0] = 1.0f;
				tissuepriors[k-1][1] = tissues[c][k];
				tissuepriors[k-1][2] = 2.0f*tissues[c][k]*tissues[c][k]-1.0f;
				imagevalues[k-1] = centroids[c][k];
			}
			...
			for (int i=0;i<3;i++) {
				coeffs[i] = 0.0f;
				for (int j=0;j<3;j++) {
					coeffs[i] += tissuepriors[j][i]*imagevalues[j];
				}
			}
